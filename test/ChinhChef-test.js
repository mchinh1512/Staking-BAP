...
contract('ChinhChef', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
      ...
      await this.chinh.transferOwnership(this.chef.address, { from: minter });
  ...
  it('real case', async () => {
    ...
    await this.chinh.approve(this.chef.address, '1000', { from: alice });
    await this.chef.pendingChinh('20', { from: alice });
    await this.chef.pendingChinh('0', { from: alice });
    await this.chef.pendingChinh('0', { from: alice });
    await this.chef.pendingChinh('0', { from: alice });
    assert.equal((await this.chinh.balanceOf(alice)).toString(), '993');
  ...
  it('deposit/withdraw', async () => {
    ...
    assert.equal((await this.chinh.balanceOf(alice)).toString(), '999');
    assert.equal((await this.chinh.balanceOf(dev)).toString(), '100');
    ...
    assert.equal((await this.chinh.balanceOf(bob)).toString(), '125');
  ...
  it('staking/unstaking', async () => {
    ...
    await this.chinh.approve(this.chef.address, '250', { from: alice });
    await this.chef.pendingChinh('240', { from: alice }); //3
    assert.equal((await this.chinh.balanceOf(alice)).toString(), '10');
    await this.chef.pendingChinh('10', { from: alice }); //4
    assert.equal((await this.chinh.balanceOf(alice)).toString(), '249');
    await this.chef.withdraw(250);
    assert.equal((await this.chinh.balanceOf(alice)).toString(), '749');
  ...
  it('updaate multiplier', async () => {
    ...
    await this.chinh.approve(this.chef.address, '100', { from: bob });
    await this.chef.pendingChinh('50', { from: alice });
    await this.chef.pendingChinh('100', { from: bob });

    await this.chef.updateMultiplier('0', { from: minter });

    await this.chef.pendingChinh('0', { from: alice });
    await this.chef.pendingChinh('0', { from: bob });
    await this.chef.deposit(1, '0', { from: alice });
    await this.chef.deposit(1, '0', { from: bob });

    assert.equal((await this.chinh.balanceOf(alice)).toString(), '700');
    assert.equal((await this.chinh.balanceOf(bob)).toString(), '150');
    ...
    await this.chef.withdraw('50', { from: alice });
    await this.chef.withdraw('100', { from: bob });
